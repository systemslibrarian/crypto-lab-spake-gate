 ...
 *
 * This lab is a live example of why the first half matters. `style.css` defines
 * `--border-control` with a comment saying it exists for "control boundaries
 * only" — and applied it to exactly ONE selector, `.pw-input`. Every button on
 * the page (`.btn`, `.chip-btn`, and the `.hex` copy chips that carry every
 * scalar, curve point, MAC and session key this lab computes) drew its edge
 * from `--border` or `--border-strong`, which are SURFACE dividers. The token
 * was right; nothing was checking where it landed.
 *
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * Sample each gradient layer at the point being judged rather than at its
     ...
     * layer (`url()`, `none`) is unmeasurable and paints nothing. This lab
     * declares no gradients today; the sampler is shared verbatim with
     * `contrast.ts` so the two oracles cannot drift into disagreeing about what
     ...
     *
     * A driven pass here walks every `.hex` chip on the page — one per scalar,
     * curve point, MAC and session key, across two handshake columns, the
     * known-answer panel, the mask stepper and both stolen-database columns —
     * each re-walking the same ancestors up to `<body>`. Nothing mutates the DOM
     * during a pass, so the cached values cannot go stale.
     */
    const styleCache = new Map<Element, CSSStyleDeclaration>();

    // The paint core above is shared verbatim with `contrast.ts`; these are the
    // pieces of it this file does not call directly, referenced so
    // `noUnusedLocals` does not reject the copy.
    void splitTopLevel;
    void over;
    void fade;
    void luminance;
     ...
     * what a page paints. A lab elsewhere in this fleet reports `rgba(0,0,0,0)`
     * for both `html` and `body` because its dark canvas is two radial
     * gradients; the walk therefore found nothing opaque, fell through to WHITE,
     * and reported a bright disclosure triangle at 3.95:1 against a page that is
     * actually near-black. The tell was that the number came out IDENTICAL in
     * both themes — a ratio that cannot depend on the theme is not measuring the
    ...
    const CONTROL = [
      ...
      '[role=radio]',
      '[role=slider]',
      // A LINK STYLED AS A BUTTON is a UI component under 1.4.11 and is easy to
      // miss: the shared header's Menu and GitHub controls are
      // `<a class="cl-btn">`, and with no `a` in this list nothing would judge
